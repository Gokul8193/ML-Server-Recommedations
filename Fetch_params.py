...
def collect_quality_scores(client, customer_id, query):
                ...
                for row in batch.results:
                    if query_name == "interaction_metrics":
                        ...
                        row_data = {
                            ...
                            "ctr": row.metrics.ctr,
                            "conversions_Master": row.metrics.conversions,
                            "cost_per_conversion_usd": row.metrics.cost_per_conversion / 1_000_000,
...
def collect_metrics(client, customer_ids, queries):
    inputs = [(client, cid, q, name) for cid in customer_ids for name, q in queries]
...
def main(client, customer_ids, start_date_str, end_date_str):
    # Use function for updating query date variables
    def update_query_dates(query):
        return query.replace("{START_DATE}", start_date_str).replace("{END_DATE}", end_date_str)
    
    print("🔍 Collecting Estimated Quality Scores...")
    qs_query = update_query_dates(QUALITY_SCORE_QUERY)
    for cid in customer_ids:
        collect_quality_scores(client, cid, qs_query)

    # Prepare all query tuples with updated dates
    metric_queries = [
        ("MASTER_CAMPAIGN_AD_GROUP_QUERY", update_query_dates(MASTER_CAMPAIGN_AD_GROUP_QUERY)),
        ("interaction_metrics", update_query_dates(INTERACTION_METRICS_QUERY)),
        ("ad_performance_metrics", update_query_dates(AD_GROUP_PERFORMANCE_QUERY)),
        ("device_performance", update_query_dates(DEVICE_PERFORMANCE_QUERY)),
        ("keyword_performance", update_query_dates(KEYWORD_PERFORMANCE_QUERY)),
        ("ad_schedule", update_query_dates(AD_SCHEDULE_QUERY)),
        ("keyword_metadata", update_query_dates(KEYWORD_METADATA_QUERY)),
        ("search_term_report", update_query_dates(SEARCH_TERM_QUERY)),
        ("geographic_report", update_query_dates(GEOGRAPHIC_REPORT_QUERY)),
        ("campaign_bidding_strategy", update_query_dates(CAMPAIGN_BIDDING_STRATEGY_QUERY)),
        ("campaign_location", CAMPAIGN_LOCATION_QUERY),
        ("campaign_negative_keywords", CAMPAIGN_NEGATIVE_KEYWORDS_QUERY),
        # ("user_list", USER_LIST_QUERY),  # commented out
        ("ad_group_ad", update_query_dates(AD_GROUP_AD_QUERY)),
    ...
    collect_metrics(client, customer_ids, metric_queries)
