...
# -------------------------------------------
# 0. Load LLM-generated keywords for input parameters
# -------------------------------------------
genai_path = "genai.json"
if os.path.isfile(genai_path):
    with open(genai_path, "r") as f:
        genai_keywords = json.load(f)
    # Use conversion and click keywords for input parameters
    conversion_positive = genai_keywords["keywords_for_increasing_conversions_and_decreasing_cost_per_conversion"]
    conversion_negative = genai_keywords["negative_keywords_for_increasing_conversions_and_decreasing_cost_per_conversion"]
    click_positive = genai_keywords["keywords_for_increasing_clicks_and_decreasing_cpc"]
    click_negative = genai_keywords["negative_keywords_for_increasing_clicks_and_decreasing_cpc"]
else:
    conversion_positive = []
    conversion_negative = []
    click_positive = []
...
# ----------------------------
# Filter only campaign_id = "851136216"
# ----------------------------
df['campaign_id'] = df['campaign_id'].astype(str)
df = df[df['campaign_id'] == '851136216'].reset_index(drop=True)
...
# Use LLM-generated keywords if available
# -------------------------------------------
explicit_values = {
    ...
    'Negative_keywords': conversion_negative + click_negative if (conversion_negative or click_negative) else ['NONE', 'COMPETITOR', 'BRAND_PROTECTION', 'LOCATION_BLOCK'],
    'Keyword_Quality_Keyword': conversion_positive + click_positive if (conversion_positive or click_positive) else ['best shoes', 'running shoes', 'discount shoes', 'sport shoes', 'winter boots'],
    'Device_performance_Cost_per_Conversion_usd': [0.5, 1.0, 1.5, 2.0],
    'search_term_search_term': click_positive if click_positive else ['cheap shoes', 'shoe store near me', 'online shoe deals', 'sports footwear'],
    'bid_Adjustments_device_type': ['DESKTOP', 'MOBILE', 'TABLET'],
    'Keyword_performance_Keyword_Text': click_positive if click_positive else ['buy shoes', 'shoe sale', 'sneaker deals'],
    'Keyword_performance_Keyword_Match_Type': ['EXACT', 'BROAD', 'PHRASE'],
    'keywords_meta_keyword_text': conversion_positive if conversion_positive else ['athletic shoes', 'formal shoes', 'kids shoes'],
